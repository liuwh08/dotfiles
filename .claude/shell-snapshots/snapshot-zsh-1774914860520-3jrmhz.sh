...
hg_dirty () {
	if [[ -z "$ZSH_THEME_HG_PROMPT_DIRTY" && -z "$ZSH_THEME_HG_PROMPT_CLEAN" ]]
	then
		return
	fi
	local hg_status
	...
	then
		if ! hg_status="$(hg status -q 2>/dev/null)" 
		then
			return
		fi
	else
		if ! hg_status="$(hg status 2>/dev/null)" 
		then
			return
		fi
	fi
	if command grep -Eq '^\s*[ACDIMR!?L].*$' <<< "$hg_status"
	then
...
hgic () {
	hg incoming "$@" | grep --color=auto --exclude-dir={.bzr,CVS,.git,.hg,.svn,.idea,.tox,.venv,venv} "changeset" | wc -l
}
hgoc () {
	hg outgoing "$@" | grep --color=auto --exclude-dir={.bzr,CVS,.git,.hg,.svn,.idea,.tox,.venv,venv} "changeset" | wc -l
}
in_hg () {
	hg_root > /dev/null
}
...
if ! (unalias rg 2>/dev/null; command -v rg) >/dev/null 2>&1; then
  ...
  if [[ -n $ZSH_VERSION ]]; then
    ARGV0=rg /home/liuwh/.local/share/claude/versions/2.1.87 "$@"
  elif [[ "$OSTYPE" == "msys" ]] || [[ "$OSTYPE" == "cygwin" ]] || [[ "$OSTYPE" == "win32" ]]; then
    ARGV0=rg /home/liuwh/.local/share/claude/versions/2.1.87 "$@"
  elif [[ $BASHPID != $$ ]]; then
    exec -a rg /home/liuwh/.local/share/claude/versions/2.1.87 "$@"
  else
    (exec -a rg /home/liuwh/.local/share/claude/versions/2.1.87 "$@")
  fi
}
fi
export PATH=/home/liuwh/.npm-global/bin\:/home/liuwh/.npm-global/bin\:/home/liuwh/.npm-global/bin\:/home/liuwh/.npm-global/bin\:/usr/local/sbin\:/usr/local/bin\:/usr/sbin\:/usr/bin\:/sbin\:/bin\:/usr/games\:/usr/local/games\:/snap/bin\:/home/liuwh/.local/bin\:/usr/local/go/bin\:/home/liuwh/Library/Python/3.7/bin\:/usr/local/google/home/liuwh/bin\:/google/src/files/head/depot/google3/third_party/javascript/node_modules/typescript/stable/bin\:/home/liuwh/.local/bin\:/usr/local/go/bin\:/home/liuwh/Library/Python/3.7/bin\:/usr/local/google/home/liuwh/bin\:/google/src/files/head/depot/google3/third_party/javascript/node_modules/typescript/stable/bin
